...
# 1. Model Service
DEFAULT_MODEL_SERVICE="system.ai.claude-sonnet-4-5"
echo "Enter the Unity Catalog model service to invoke via the Unity Gateway (3-part name, e.g. system.ai.claude-sonnet-4-5)"
echo "Press Enter to use default: $DEFAULT_MODEL_SERVICE"
read -r MODEL_SERVICE

if [ -z "$MODEL_SERVICE" ]; then
    MODEL_SERVICE="$DEFAULT_MODEL_SERVICE"
    echo "Using default model service: $MODEL_SERVICE"
fi

# Model services are Unity Catalog securables (3-part names). There's no
# verified CLI command to check for their existence here. Existence and the
# required EXECUTE / USE CATALOG / USE SCHEMA grants are verified when the
# app actually calls the Unity Gateway at deploy/runtime.
MODEL_SERVICE_CHECK=""
MODEL_SERVICE_EXISTS=1

# databricks.yml's model_service_name variable is documented as
# non-functional (no model-service DAB/Apps resource type exists yet), so
# there's no placeholder there to substitute; DATABRICKS_MODEL_SERVICE in
# .env is what actually drives the app.

# Update .env with the model service
if grep -q "DATABRICKS_MODEL_SERVICE=" .env; then
    if [[ "$OSTYPE" == "darwin"* ]]; then
        sed -i '' "s|DATABRICKS_MODEL_SERVICE=.*|DATABRICKS_MODEL_SERVICE=$MODEL_SERVICE|" .env
    else
        sed -i "s|DATABRICKS_MODEL_SERVICE=.*|DATABRICKS_MODEL_SERVICE=$MODEL_SERVICE|" .env
    fi
else
    echo "DATABRICKS_MODEL_SERVICE=$MODEL_SERVICE" >> .env
fi
echo "✓ Model service configured"
...
echo "It requires your model service to have an associated MLflow experiment."
echo
...
if is_feedback_enabled; then
    ...
    # Model services have no discoverable link to an MLflow experiment (see
    # MODEL_SERVICE_EXISTS above), so this never auto-detects here.
    EXPERIMENT_ID=""
    if [ $MODEL_SERVICE_EXISTS -eq 0 ]; then
        EXPERIMENT_ID=$(echo "$MODEL_SERVICE_CHECK" \
            | jq -r '(.tags // []) | map(select(.key == "MONITOR_EXPERIMENT_ID")) | .[0].value // empty' \
            2>/dev/null || true)
    fi

    if [ -n "$EXPERIMENT_ID" ]; then
        echo "✓ Found MLflow experiment (ID: $EXPERIMENT_ID) linked to '$MODEL_SERVICE'."
        echo
        ...
        if prompt_yes_no "Do you want to enable the feedback widget?" "Y"; then
            ...
            echo "To enable it later, set MLFLOW_EXPERIMENT_ID directly in .env, and uncomment the experiment block in databricks.yml and app.yaml."
        fi
    else
        echo "ℹ️  No MLflow experiment could be auto-detected for model service '$MODEL_SERVICE'."
        echo "   Set MLFLOW_EXPERIMENT_ID in .env manually if you have an experiment ID to link."
    fi
fi
