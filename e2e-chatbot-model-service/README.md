<a href="https://docs.databricks.com/aws/en/ai-gateway/query-model-services">
  <h1 align="center">Databricks Model Service Chat Template</h1>
</a>

<p align="center">
    A chat application template for interacting with a Unity Catalog model service via the Databricks Unity Gateway, built with ExpressJS, React, Vercel AI SDK, Databricks authentication, and optional Lakebase (database) integration.
</p>
...
This template provides a fully functional chat app for Unity Catalog model services (queried via the Databricks Unity Gateway) deployed on Databricks,
but has some [known limitations](#known-limitations) for other use cases. Work is in progress on addressing these limitations.

## Features

- **Databricks Model Service Integration**: Direct connection to a Unity Catalog model service via the Unity Gateway's OpenAI-compatible `/ai-gateway/mlflow/v1/chat/completions` endpoint
...
1. **Databricks model service**: you need access to a Databricks workspace containing a Unity Catalog model service to chat with (a 3-part UC name, e.g. `system.ai.claude-sonnet-4-5`).
...
1. **Clone the repo**:
   ...
   cd e2e-chatbot-model-service
   ```
2. **Databricks authentication**: Ensure auth is configured as described in [Prerequisites](#prerequisites).
3. **Specify the model service**: Set `DATABRICKS_MODEL_SERVICE` in `app.yaml` to the 3-part Unity Catalog name of the model service to chat with (e.g. `system.ai.claude-sonnet-4-5`). There is no bundle resource for model services yet (see the NOTE in `databricks.yml`), so this is a plain literal env value rather than a `valueFrom` resource binding. Separately, grant the app's service principal `EXECUTE` on the model service, plus `USE CATALOG` / `USE SCHEMA` on its containing catalog/schema, via Unity Catalog grants.

   The optional commented-out sections in `databricks.yml` allow you to enable:
   ...
   - **User feedback collection** — uncomment the optional `TODO` experiment block and set the experiment ID. Also requires a database (both database `TODO` blocks must be uncommented). See [Feedback Collection](#feedback-collection) for details. **Tip:** `./scripts/quickstart.sh` can provision the database for you, but the experiment ID still needs to be set manually (see [Feedback Collection](#feedback-collection)).
...
(`model_service_name` isn't passed via `--var` here since it isn't wired into a bundle resource -- set `DATABRICKS_MODEL_SERVICE` directly in `app.yaml` instead.)
...
2. **Run the quickstart script**:
   ...
   > **Note:** `quickstart.sh` prompts directly for your Unity Catalog model service name (3-part UC name) and writes it to `DATABRICKS_MODEL_SERVICE` in `.env` for local development. It cannot validate that the model service exists, and it does not update `app.yaml` -- set `DATABRICKS_MODEL_SERVICE` there yourself before deploying, per [Prerequisites](#prerequisites).
   ...
   - **Configure the model service** - Prompts for your model service name (see note above)
...
2. **Set up environment variables**:
   ...
   Address the TODOs in `.env`, specifying your Databricks CLI profile, model service name (`DATABRICKS_MODEL_SERVICE`), and database connection details.
...
The chat app supports optional thumbs up/down feedback on assistant messages. When enabled, feedback is stored as [MLflow assessments](https://docs.databricks.com/aws/en/generative-ai/agent-framework/chat-app) on the traces emitted by your model service, making it easy to review and act on in the MLflow UI.

Feedback is **disabled by default**. A "Feedback disabled" badge appears in the header when it is not configured.

> **Note:** Feedback vote persistence (restoring thumbs up/down state on page reload) requires a database. The quickstart script can provision the database in one step; the feedback experiment ID must still be set manually (see below).

### Recommended: use the quickstart script for the database

Run the interactive setup script to provision the Lakebase database needed for feedback vote persistence:

```bash
./scripts/quickstart.sh
```

The script can't auto-detect a linked MLflow experiment for a model service, so it won't uncomment the feedback `TODO` block for you. It will:
1. Prompt for your model service name and offer to enable persistent chat history, uncommenting both database `TODO` blocks in `databricks.yml`
2. Tell you to set `MLFLOW_EXPERIMENT_ID` manually, since it currently can't auto-detect an experiment for a model service

Follow the manual steps below to find your experiment ID and uncomment the feedback `TODO` block, then run `databricks bundle deploy`.
...
Find or create the MLflow experiment you want to link (e.g. in the MLflow Experiment Tracking UI), and note its experiment ID.
